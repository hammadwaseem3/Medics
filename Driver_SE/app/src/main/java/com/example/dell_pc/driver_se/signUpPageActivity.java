                ...
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        ...
                        if (!task.isSuccessful()) {
                            ...
                            myRef = database.getReference("Ambulance/"+task.getResult().getUser().getUid()+"/num");
                            String contactNumber = ((EditText) findViewById(R.id.signup_num)).getText().toString().trim();
                            myRef.setValue(contactNumber);

                            myRef = database.getReference("Ambulance/"+task.getResult().getUser().getUid()+"/name");
                            String name = ((EditText) findViewById(R.id.signup_name)).getText().toString().trim();
                            myRef.setValue(name);

                            myRef = database.getReference("Ambulance/"+task.getResult().getUser().getUid()+"/id");
                            myRef.setValue(task.getResult().getUser().getUid());

                            myRef = database.getReference("Ambulance/"+task.getResult().getUser().getUid()+"/req_user_id");
                            myRef.setValue("null");
